...
class BlockChain {
    ...
    validateBlock(block) {
        const hash = sha256(`${block.nonce}${block.previousBlock}${block.transactions}${block.timeStamp}`);
        console.log(hash)
        if(block.hash === hash) {
...
// esto esta realizado para entender la estructura de blockchain de una forma mas simple // 